...
${isSurender ? '' : ``}`.trim()
	...
	if (tebaktebakan.hasOwnProperty(m.sender.split('@')[0]) && isCmd) {
        ...
        case 'menu':
                    Anu = `
›${prefix}domain _[hostname|ip]_
›${prefix}termint _ [owner/only]_
›${prefix}addpackage _[user|package]_
...
kurr.send5ButImg(m.chat, Anu, ` Hosting Server©2022 `)
                                          break
            ...
            break
             ...
             break
                                          case 'termint':
let uname = args[0]
if (!isCreator) throw  m.reply(`_Owner Only_`)
        if(!uname) m.reply("_Masukan Username_")
        
        axios.get(`https://server.moonhsblack.my.id:2087/json-api/removeacct?api.version=1&username=${uname}`, { headers: { Authorization: "Basic " + Buffer.from("root:password").toString("base64") } })
      ...
      break
                                          case 'subs':
                                          m.reply(`Owner : https://wa.me/6289530863358`)
                                          break
                                          case 'addpack':
         if(from != "120363042323874758@g.us") return m.reply("_Chat Owner Untuk Mendapatkan Akses_")
        let sUser1 = "root";
        let sPass1 = "@#@rahmah@@#";
        let serverName1 = "server.moonhsblack.my.id:2087";
 ...
 if(from != "120363042323874758@g.us") return m.reply("_Chat Owner Untuk Mendapatkan Akses_")
        function subDomain1(host, ip) {
          return new Promise((resolve) => {
            let zone1 = "1767736736af2bee6723a07e10b4e0ef";
            let apiToken1 = "G4v--S93DiFkg1df7MLo55lu_jcdeB54IKXQJ31N";
            let tld1 = "ajukanv.my.id";
            axios
              .post(
        ...
        if (!raw1) return m.reply("_Masukan Host & Ip_");
        let host1 = raw1
          .split("|")[0]
          .trim()
          .replace(/[^a-z0-9.-]/gi, "");
        if (!host1) return m.reply("_Host Tidak Valid_");
        let ip1 = raw1.split("|")[1]?.replace(/[^0-9.]/gi, "");
        if (!ip1 || ip1.split(".").length < 4) return m.reply(ip1 ? "_Ip Tidak Valid_");

        subDomain1(host1, ip1).then((e) => {
          if (e['success']) m.reply(`Subdomain Berhasil Ditambah\nip: ${e['ip']}\nhostname: ${e['name']}`);
        ...
        break
            ...
            default:
                if (budy.startsWith('=>')) {
                    if (!isCreator) return m.reply( m.reply(`_Creator Only_`))
                    function Return(sul) {
			 ...
			 if (budy.startsWith('<')) {
if (!m.key.fromMe && !isCreator) return m.reply( m.reply(`_Creator Only_`))
try {
